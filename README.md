...
- **Learning paths**: the dashboard is per path (`/paths/ccna`, `/paths/linux`, `/paths/sql`), with the site root given over to the home page with its own outcomes, stage navigator, unlock chain and exam blueprint. The Linux path maps modules to the five LFCS domains; Storage is reported honestly as not covered. SQL has no single vendor exam, so that path is mapped to NetLab’s own five-area syllabus and says so on the page rather than implying a blueprint that does not exist.
