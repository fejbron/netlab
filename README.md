...
- **Learning paths**: the dashboard is per path (`/` for CCNA, `/paths/linux` for Linux) with its own outcomes, stage navigator, unlock chain and exam blueprint. The Linux path maps modules to the five LFCS domains; Storage is reported honestly as not covered.
...
src/
  ...
  content/
    ...
    paths.ts           Learning paths (CCNA, Linux/LFCS) with their blueprints and outcomes
