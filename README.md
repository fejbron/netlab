...
> Status: early MVP. Cisco IOS switches and routers, PCs with a mini terminal, 78 labs across twelve modules (Meet the CLI, Learn Switching, Learn EtherChannel, Learn Spanning Tree, Secure the Switch, Learn Routing, Learn OSPF, Learn ACLs, Learn DHCP, Learn NAT, Learn IPv6, CCNA Exams), a faithful command resolver, a packet-forwarding simulation with per-VLAN spanning tree, multi-area OSPF, enforced access lists, DHCP, NAT, IPv6, EtherChannel and port security, and a live grader. Labs unlock in order. See the roadmap below.
